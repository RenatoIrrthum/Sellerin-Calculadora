    ...
    <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
      <div className="px-6 py-4 border-b border-slate-100">
        <h2 className="text-base font-bold text-slate-800 m-0">
          Comparativo — todos os marketplaces
        </h2>
        <p className="text-xs text-slate-500 mt-0.5 m-0">
          {calcMode === 'suggested_price'
            ? 'Ordenado pelo menor preco sugerido'
            : 'Ordenado pela maior margem'}
        </p>
      </div>
        ...
        <table className="w-full text-sm">
          <thead>
            <tr className="bg-slate-50 border-b border-slate-100">
              ...
              <th className="text-right px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">
                {calcMode === 'suggested_price' ? 'Preco sugerido' : 'Preco'}
              </th>
              <th className="text-right px-4 py-3 text-xs font-semibold text-slate-500 uppercase tracking-wide">
                Lucro
              </th>
          ...
          <tbody>
            {results.map((r, i) => {
              ...
              const logoTextColor = LOGO_TEXT_COLORS[r.marketplaceId] ?? '#FFFFFF';
              return (
                <tr
                  key={r.marketplaceId}
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-2">
                      ...
                      <div>
                        ...
                        {r.plan && (
                          <p className="text-xs text-slate-400 m-0">{r.plan}</p>
                        )}
                      </div>
                      {i === 0 && (
                  ...
                  <td className="px-4 py-3 text-right">
                    ...
                    <span className="text-slate-400 text-xs ml-1">
                      ({formatPercent(r.effectiveFeeRate, 1)})
                    </span>
                  </td>
                  <td className="px-4 py-3 text-right">
                    {r.shippingPaidBySeller === 0 ? (
                      ...
                      <span className="text-red-500 font-medium">
                        {formatCurrency(r.shippingPaidBySeller)}
                      </span>
                    )}
                  </td>
                  <td className="px-4 py-3 text-left">
                    <span className="text-xs text-slate-500">{r.plan ?? '-'}</span>
                  </td>
                </tr>
              );
            })}
